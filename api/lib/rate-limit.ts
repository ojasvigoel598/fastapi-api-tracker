...
export class FixedWindowRateLimiter {
  ...
  /**
   * Record one attempt against a per-window budget and report whether this
   * call was within it. Unlike `recordFailure` (auth failures), `consume`
   * counts successful calls too, so it can gate high-volume surfaces
   * (webhook ingest, AI analysis, export) with a simple max-calls-per-window.
   */
